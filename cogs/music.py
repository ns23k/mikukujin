...
language = "en"  # en/tc
useEmbed = False
loc = json.load(open(os.getcwd() + "\\cogs\\" + f"music_{language}.json", "r", encoding="utf-8"))
...
async def _join(ctx, from_cmd=False, author=None):
    ...
    if from_cmd:
        await respond(
            ctx, loc["messages"]["joined"].format(destination), color=0x1EFF00
        )
    ...
    if isBotInChannel(ctx):
        if isBotAndUserInSameChannel(ctx, ctx.author):
            ...
            return await respond(
                ctx, loc["messages"]["enter_before_play"], color=0xFF0000
            )
    else:
        ...
        if not ctx.voice_state.voice:
            return await respond(
                ctx, loc["messages"]["cannot_join_unk"], color=0xFF0000
            )

    try:
        if search_msg is None:
            searching_msg = await respond(
                ctx, loc["messages"]["searching"].format(search)
            )
        else:
        ...
        if "/playlist?" in search and ("youtu.be" in search or "youtube.com" in search):
            partial = functools.partial(
                SongSource.ytdl_playlist.extract_info, search, download=False
            )
            ...
            if data is None:
                await editMessage(
                    searching_msg,
                    discord.Embed(
                        title=loc["not_found"].format(search), color=0xFF0000
                    ),
                )
            ...
            await editMessage(
                searching_msg,
                discord.Embed(
                    title=loc["messages"]["added_multiple_song"].format(songs + 1),
                    color=0x1EFF00,
                ),
            )
        else:
            ...
            if "youtube" not in domain and "youtu.be" not in domain:
                ...
                try:
                    partial = functools.partial(
                        SongSource.ytdl.extract_info, search, download=False
                    )
                    data = await loop.run_in_executor(None, partial)
                    if "entries" in data:
                        if len(data["entries"]) > 0:
                            ...
                            return await editMessage(
                                searching_msg,
                                discord.Embed(
                                    title=loc["not_found"].format(search),
                                    color=0xFF0000,
                                ),
                            )
                                ...
                                "duration": int(
                                    float(
                                        subprocess.check_output(
                                            f'ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "{search}"',
                                            shell=True,
                                        )
                                        .decode("ascii")
                                        .replace("\r", "")
                    ...
                    return await editMessage(
                        searching_msg,
                        discord.Embed(
                            title=loc["messages"]["cannot_add_invalid_file"],
                            color=0xFF0000,
                        ),
                    )
                return await editMessage(
                    searching_msg,
                    discord.Embed(
                        title=loc["messages"]["added_song"].format(
                            title.replace("_", "\\_")
                        ),
                        color=0x1EFF00,
                    ),
                ...
                try:
                    ...
                    return await editMessage(
                        searching_msg,
                        discord.Embed(
                            title=loc["messages"]["error"].format(message),
                            color=0xFF0000,
                        ),
                    )
                if "entries" in data:
                ...
                await editMessage(
                    searching_msg,
                    discord.Embed(
                        title=loc["messages"]["added_song"].format(data["title"]),
                        color=0x1EFF00,
                    ),
                )
        ...
        await editMessage(
            searching_msg,
            discord.Embed(
                title=loc["messages"]["error"].format(str(e)), color=0x1EFF00
            ),
        )
    ...
    embed = (
        ...
        .add_field(
            name=loc["queue_embed"]["embed_body"].format(
                len(data), parse_duration(getTotalDuration(data))
            ),
            value=queue,
        )
...
class SongSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(
        ...
        partial = functools.partial(
            self.ytdl.extract_info, keyword, download=False, process=False
        )
        data = await loop.run_in_executor(None, partial)
        ...
        if src == "ffmpeg":
            ...
            partial = functools.partial(
                self.ytdl.extract_info, webpage_url, download=False
            )
        ...
        if seek is not None:
            ...
            return self(
                ctx,
                discord.FFmpegPCMAudio(info["url"], **seek_option, executable=imageio_ffmpeg.get_ffmpeg_exe()),
                data=info,
                ffmpeg=(src == "ffmpeg"),
            )
        else:
            return self(
                ctx,
                discord.FFmpegPCMAudio(info["url"], **FFMPEG_OPTIONS, executable=imageio_ffmpeg.get_ffmpeg_exe()),
                data=info,
                ffmpeg=(src == "ffmpeg"),
            )
        ...
        embed = (
            ...
            .add_field(
                ...
                value=(
                    ...
                    if status == "play" or self.source.duration == "Unknown"
        ...
        if "local@" in url:
            ...
            if seek is not None:
                return Song(
                    SongSource(
                        ...
                        discord.FFmpegPCMAudio(
                            url, before_options="-ss " + parse_duration_raw(seek), executable=imageio_ffmpeg.get_ffmpeg_exe()
                        ),
                        data={
                            "duration": duration,
                            "title": title,
                ...
                return Song(
                    SongSource(
                        ctx,
                        discord.FFmpegPCMAudio(url, executable=imageio_ffmpeg.get_ffmpeg_exe()),
                        data={
                            "duration": duration,
                            "title": title,
                            "url": "local@" + url,
            ...
            try:
                result = Song(
                    await SongSource.create_source(
...
class PlayerControlView(discord.ui.View):
    ...
    async def loopqueue(self, interaction, button):
        if interaction.user not in self.voice_state.voice.channel.members:
            return await interaction.response.send_message(
                loc["messages"]["not_in_same_voice"], ephemeral=True
            )
...
class Music(commands.Cog):
    ...
    async def cog_command_error(self, ctx, error):
        ...
        await ctx.send(
            "Stacktrace",
            file=discord.File(
                io.BytesIO(traceback.format_exc().encode("utf-8")), f"error.txt"
            ),
        )
        if not (
                str(error)
    ...
    @commands.hybrid_command(description=loc["descriptions"]["join"])
    async def join(self, ctx):
        await _join(ctx, from_cmd=True, author=ctx.author)

    @commands.hybrid_command(
        description=loc["descriptions"]["leave"]
    )
    ...
    @commands.hybrid_command(description=loc["descriptions"]["volume"])
    async def volume(
            self, ctx,
            volume: int = None
    ):
        ...
        if volume is not None:
            ...
            if 0 > volume or volume > 200:
                return await respond(
                    ctx, loc["messages"]["volume_invalid_range"], color=0xFF0000
                )
            # The volume should be divided by 100
            ctx.voice_state.volume = volume / 100
            await respond(
                ctx, loc["messages"]["updated_volume"].format(volume), color=0x1EFF00
            )
        else:
            # Return the current volume
            return await respond(
                ctx,
                loc["messages"]["current_volume"].format(
                    int(ctx.voice_state.volume * 100)
                ),
                color=0x1EFF00,
            )

    @commands.hybrid_command(description=loc["descriptions"]["now"])
    ...
    @commands.hybrid_command(description=loc["descriptions"]["pause"])
    ...
    @commands.hybrid_command(description=loc["descriptions"]["resume"])
    ...
    @commands.hybrid_command(description=loc["descriptions"]["stop"])
    ...
    @commands.hybrid_command(description=loc["descriptions"]["skip"])
    ...
    @commands.hybrid_command(description=loc["descriptions"]["queue"])
    async def queue(
            self, ctx, *, page: int=None
    ):
        # Shows the queue, add page number to view different pages
        if not (await checkUserAndBotChannel(ctx)):
            return
    ...
    @commands.hybrid_command(description=loc["descriptions"]["shuffle"])
    ...
    @commands.hybrid_command(description=loc["descriptions"]["remove"])
    async def remove(
            self,
            ctx,
            index: int = None
        ...
        try:
            ...
            return await respond(
                ctx, loc["messages"]["invalid_song_number"], color=0xFF0000
            )
    ...
    @commands.hybrid_command(description=loc["descriptions"]["loop"])
    async def loop(self, ctx):
        ...
        await respond(
            ctx,
            loc["messages"][
                ("enable" if ctx.voice_state.loop else "disable") + "_loop"
                ],
        )
        if ctx.voice_state.message:
            await ctx.voice_state.update()

    @commands.hybrid_command(description=loc["descriptions"]["play"])
    async def play(
            self,
            ctx,
            search: str = None,
    ...
    @commands.hybrid_command(description=loc["descriptions"]["search"])
    async def search(
            self,
            ctx,
            keyword: str = None,
        ...
        if keyword == None:
            return await respond(
                ctx, loc["messages"]["play_no_keyword"], color=0xFF0000
            )
    ...
    @commands.hybrid_command(description=loc["descriptions"]["musicreload"])
    ...
    @commands.hybrid_command(
      description=loc["descriptions"]["loopqueue"]
    )
    async def loopqueue(self, ctx):
        ...
        await respond(
            ctx,
            loc["messages"][
                ("enable" if ctx.voice_state.loopqueue else "disable") + "_loopqueue"
                ],
        )
        if ctx.voice_state.message:
            await ctx.voice_state.update()

    @commands.hybrid_command(description=loc["descriptions"]["playfile"])
    async def playfile(
            self,
            ctx,
            attachment: str = None,
        ...
        try:
            ...
            return await respond(
                ctx, loc["messages"]["cannot_add_invalid_file"], color=0xFF0000
            )
    ...
    async def runningservers(self, ctx):
        ...
        if ctx.author.id in authors:
            ...
            return await respond(
                ctx,
                embed=discord.Embed(
                    title=loc["messages"]["runningservers"].format(str(server_count)),
                    description=desc[:-1],
                ),
            )

    @commands.hybrid_command(description=loc["descriptions"]["seek"])
    async def seek(
            self, ctx, seconds: int
    ):
        if not (await checkUserAndBotChannel(ctx)):
            return
        ...
        if ctx.voice_state.is_playing and ctx.voice_state.voice.is_playing():
            ...
            try:
                ...
                return await respond(
                    ctx, loc["messages"]["parse_second_failed"], color=0xFF0000
                )
            if seconds is None:
                return await respond(
                    ctx, loc["messages"]["no_second_parse"], color=0xFF0000
                )
            ...
            await respond(
                ctx, loc["messages"]["seekto"].format(seconds), color=0x1EFF00
            )
        else:
            await respond(ctx, loc["messages"]["no_playing"], color=0xFF0000)
